...
def visualize_pipeline_results(chi_scores, candidate_df, final_features, target_col):
    """Diagnostic Dashboard for Chi-Squared and AMINO results."""
    sns.set_theme(style="whitegrid")
    fig, axes = plt.subplots(1, 3, figsize=(24, 7))

    # --- Panel 1: The Signal Landscape (Scree Plot) ---
    sorted_scores = chi_scores.sort_values(ascending=False)
    axes[0].plot(range(len(sorted_scores)), sorted_scores.values, color='#333333', lw=1)
    ...
    if len(final_features) > 0:
        ...
        axes[0].scatter(final_indices, sorted_scores.iloc[final_indices], color='red', s=50, zorder=5, label='AMINO Selected')
        axes[0].legend()
        
    axes[0].set_title(f"Feature Signal Strength (Chi-Squared)", fontsize=14)
    axes[0].set_xlabel("Feature Rank")
    axes[0].set_ylabel("Chi-Squared Score")
    axes[0].set_yscale('log')

    # --- Panel 2: The Mechanism (Contingency Heatmap) ---
    if len(final_features) > 0:
        top_feature = final_features[0]
        plot_df = candidate_df.copy()
        plot_df['binned'] = pd.qcut(plot_df[top_feature], q=10, duplicates='drop')
        confusion = pd.crosstab(plot_df[target_col], plot_df['binned'])
        confusion_norm = confusion.div(confusion.sum(axis=1), axis=0)
        
        sns.heatmap(confusion_norm, ax=axes[1], cmap="viridis", cbar_kws={'label': 'Density'}, annot=False)
        axes[1].set_title(f"Why '{top_feature}' Won:\nClass Separation by Value Bin", fontsize=14)
        axes[1].set_xlabel(f"Value Bins (Low -> High)")
    ...
    if len(final_features) >= 2:
        ...
        sample_df = candidate_df.sample(min(5000, len(candidate_df)))
        sns.scatterplot(
            data=sample_df, x=f1, y=f2, hue=target_col, 
            palette="deep", s=15, alpha=0.7, ax=axes[2], edgecolor=None
        )
...
def extract_candidates_only(df_iterator, target_col, candidates, stride=1):
    ...
    for chunk in df_iterator:
        ...
        cols_to_keep = list(candidates) + [target_col] + existing_meta
...
def run_feature_selection_pipeline(df_iterator_factory, target_col='class', stride=5, max_amino=10, 
                                   show_plots=True, q_bins=5, sample_rows=20000, knee_S=5.0, 
    ...
    if amino is not None and actual_max > 0:
        ...
        try:
            final_ops = amino.find_ops(ops, max_outputs=actual_max, bins=bins, distortion_filename=distortion_filename)
            ...
            final_names = candidate_features[:actual_max]
    else:
        final_names = candidate_features[:actual_max]
    ...
    if show_plots and len(valid_final_names) > 0:
        visualize_pipeline_results(chi_s, df_strided, valid_final_names, target_col)
