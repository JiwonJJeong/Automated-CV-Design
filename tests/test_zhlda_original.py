...
zhlda_mod = load_module_from_path("zhlda_mod", os.path.join('{BASE_DIR}', 'lda', '4_dimensionality_reduction', 'ZHLDA.py'))

# Prepare data matching notebook logic
df = utils.get_mpso_data()
# ZHLDA notebook uses 1, 2, 3 labels
...
"""
    ...
    for col in ['LD1', 'LD2']:
        ...
        assert min(diff_pos, diff_neg) < 1e-3, f"Values in {col} do not match reference (even with sign flip)"

    print("ZHLDA test passed.")
