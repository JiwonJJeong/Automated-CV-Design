...
class TestMHLDA:
    ...
    def test_reference_output_comparison(self):
        ...
        try:
            ...
            if not os.path.exists(input_file):
                pytest.fail("❌ Input data file (mpso.csv) not found. MHLDA reference test requires the same input data as reference notebook.")
            ...
            y = np.concatenate([np.zeros(nDataPoints)+1, np.ones(nDataPoints)+1, np.ones(nDataPoints)+2])  # MHLDA uses 1, 2, 3 labels
            df['class'] = y
            
            # STEP 5: Run MHLDA with exact same parameters as notebook
            print("🔄 Running MHLDA with exact reference parameters (num_eigenvector=2)...")
            ...
            print(f"✅ MHLDA results match reference within tolerance")
            ...
            raise AssertionError(f"MHLDA reference comparison failed: {e}") from e
    ...
    def test_reproducibility(self, sample_dataframe):
        ...
        result_iter1 = mhlda_mod.run_mhlda(sample_dataframe, num_eigenvector=1, target_col='class')
        result_df1 = next(result_iter1)
        
        np.random.seed(42)
        
        result_iter2 = mhlda_mod.run_mhlda(sample_dataframe, num_eigenvector=1, target_col='class')
